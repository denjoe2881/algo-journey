...
**PC Judge** là hệ thống cho phép giảng viên chấm bài Java của sinh viên trực tiếp trên máy tính (không cần browser). Mỗi bài trong catalog [Algo Journey](/src/content/problems/) có một folder tương ứng trong `out/pc-judge/`, chứa đủ mọi thứ để compile và chạy.
...
├── results.json        ← Kết quả lần chạy gần nhất (bao gồm coverage nếu đã đo)
└── README.txt          ← Hướng dẫn chi tiết + đề bài

out/lib/jacoco/        ← JaCoCo JARs (tự động tải, chỉ JDK cần thiết)
├── jacocoagent.jar
└── jacococli.jar
```
...
```
out/pc-judge/lru-cache/LRUCache.java   ← GHI ĐÈ file placeholder
```
...
```
Lệnh này sẽ biên dịch mã khởi tạo (code trống) của toàn bộ sinh viên, thu thập log báo lỗi và kiểm tra xem test harness có chạy bình thường hay bị crash không. File báo cáo sẽ được sinh ra ở `out/pc-judge/1_report_starter.json`.
...
# Chỉ một số bài:
npm run pc-judge:verify verify-refs coffee-decorator;lru-cache
```
...
Bất kỳ trường hợp nào sai khác (do generator lỗi, hoặc reference lỗi) sẽ lập tức báo `FAILED`. File báo cáo sinh ra tại `out/pc-judge/3_report_ref.json`.

---

### 4. Đo Code Coverage (JaCoCo)
```bash
npm run pc-judge:coverage
# Chỉ một số bài:
npm run pc-judge:coverage coffee-decorator
npm run pc-judge:coverage slug1;slug2;slug3
```

Đo **line, branch và method coverage** thực sự của bộ testcases đối với đáp án mẫu, sử dụng **JaCoCo** — công cụ coverage chuẩn của Java.

**Yêu cầu:** Chỉ cần JDK. Script tự động tải JaCoCo 0.8.13 về `out/lib/jacoco/` trong lần chạy đầu tiên (dùng `curl.exe` có sẵn trên Windows 10+).

**Flow nội bộ:**
1. Tự động tải `jacocoagent.jar` + `jacococli.jar` vào `out/lib/jacoco/` (chỉ lần đầu)
2. Copy `_solution_ref.java` → `<StudentFile>.java`
3. Compile với `-g` (debug symbols để coverage chính xác đến từng dòng)
4. Chạy `java -javaagent:jacocoagent.jar Runner` → sinh `jacoco.exec`
5. Generate `coverage.xml` từ `jacoco.exec`
6. Parse XML → ghi coverage vào `results.json` của bài (thêm field `"coverage"`)
7. Tổng hợp toàn bộ → `out/pc-judge/4_report_coverage.json`
...
**Format `4_report_coverage.json`** (tương tự `3_report_ref.json`, thêm field `coverage`):
```json
{
  "generatedAt": "2026-04-20T14:11:53.601Z",
  "jacocoVersion": "0.8.13",
  "total": 1,
  "perfect": 1,
  "failedOrCrashed": 0,
  "aggregateCoverage": {
    "lines":    { "covered": 37, "total": 37, "pct": 100 },
    "branches": { "covered": 13, "total": 14, "pct": 92.86 },
    "methods":  { "covered": 20, "total": 20, "pct": 100 }
  },
  "details": [
    {
      "slug": "coffee-decorator",
      "status": "100%_CORRECT",
      "stats": { "passed": 23, "failed": 0, "errors": 0 },
      "samples": [ ... ],
      "suspiciousTestWarning": "",
      "coverage": {
        "lines":    { "covered": 37, "missed": 0,  "total": 37, "pct": 100   },
        "branches": { "covered": 13, "missed": 1,  "total": 14, "pct": 92.86 },
        "methods":  { "covered": 20, "missed": 0,  "total": 20, "pct": 100   }
      }
    }
  ]
}
...
> **Giải thích metrics:**
> - **lines**: số dòng code được thực thi ít nhất 1 lần
> - **branches**: số nhánh điều kiện (`if/else`, `switch`, `?:`) được kiểm tra
> - **methods**: số methods được gọi
>
> Branch coverage < 100% có thể chỉ ra bộ test chưa bao quát hết tình huống biên (edge cases).
