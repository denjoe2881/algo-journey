...
export default defineTests('two-sum-sorted', (t, rng) => {
  ...
  // ── Hidden Tests ──
  t.hidden('all-same', { args: [Array(100).fill(5), 10], expected: [0, 99] }); // Two pointers start from outside
  t.hidden('not-found-large', { args: [[10, 11, 12, 13, 14, 15, 16], 100], expected: [-1, -1] });
  t.hidden('large-values', { args: [[100000000, 100000001, 100000002], 200000001], expected: [0, 1] });

  // ── Generated hidden tests (15) ──
  for (let i = 0; i < 15; i++) {
    const len = rng.int(10, 500);
    const arr = rng.sortedUniqueIntArray(len, -50000, 50000);
    ...
    // Evaluate the actual 2 pointer expectation
    let expected = [-1, -1];
    let l = 0, r = len - 1;
    while (l < r) {
      if (arr[l]! + arr[r]! === target) { expected = [l, r]; break; }
